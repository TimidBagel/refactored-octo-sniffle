...
func add_civilization(new: Civilization) -> bool:
...
func remove_civilization(to_remove: Civilization) -> bool:
...
func update_total_population() -> void:
	var count := 0
...
func _on_click_area_input_event(viewport: Viewport, event: InputEvent, shape_idx: int) -> void:
	if event.is_pressed() && event.is_action("mouse_left"):
