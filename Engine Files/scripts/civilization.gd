...
func _ready(init_pop, init_fuel, init_oxygen, init_food, init_water, init_buildings):
...
func passive_resource_consumption():
	...
	self.population -= int(self.population / 25) # placeholder replace with an actual value to subtract the population by, ideally it will be casted to an integer
